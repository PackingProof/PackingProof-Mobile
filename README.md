...
- **面单条码识别**：只认一维物流条码，二维码和指令码不会挤掉条码；同一帧出现多个码时按业务优先级处理，内置京东等平台的单号规则
- **两种工作模式**：连续扫码适合流水打包，同码停录适合一单一段的操作方式
- **录像水印**：结束录制时把水印烧录进成片，水印跟随实际录制编码（设备不支持偏好编码而回退到 H.265 / HEVC 时同样正确）
- **按单号查找与回放**：订单历史按快递单号搜索并跳到对应画面；播放页显示业务类型、分辨率、时长等信息，竖版录像的剪辑预览不变形，支持全屏铺满
- **剪辑与导出**：在手机上直接剪辑片段并导出、分享成片
- **订单语音提醒**：可播报买家留言、卖家备注、商品信息和退款状态；退款触发醒目的工业警报音（支持系统 TTS 与 Edge TTS）
- **电脑自动备份**：扫描电脑端二维码完成配对，在局域网内按备份协议自动上传录像；iOS 在后台不做清理轮询，避免被系统按 CPU 超限杀掉
- **多工位身份**：注册与心跳带上平台标识，主机按平台分配设备名；配对记录与电脑身份不符时提示重新连接
- **来源筛选**：按设备区分来源（不再笼统归为"电脑录像"），翻页后仍记住所选设备名

...
`POST /api/connections/heartbeat` 由手机按主机下发的间隔定期发送，用于刷新设备在线状态并领取主机分配的昵称。200 响应可以携带：

```json
{
  "assignedDisplayName": "安卓3",
  "computerName": "仓库电脑"
}
```

- `assignedDisplayName` 是主机分配的设备昵称；与本地显示名不同时手机立即更新并持久化，不需要用户操作
- `computerName` 是主机当前的电脑名，与注册响应里的同名字段同源；与已保存的电脑名不同且非空时，手机更新"已连接电脑"显示名并持久化，用户无需重新配对
- 响应里没有 `computerName`、或值为空白时，手机保留已保存的电脑名，不清空、不重复写盘，也不额外通知界面
- 两个字段都不变的响应不产生写盘；心跳失败或电脑离线时保留本地录像、待备份任务和已保存的连接信息
...
主机以 `sourceDeviceId` 为准识别设备，`sourceDeviceName` 只作兼容字段：昵称由主机分配、只保存最新一份映射，不会逐条写进录像记录。手机端不需要因为改名重新上传或重新配对
